...
Here's the Git diff for the centrim-docgen/centrim-docgen-0.0.1.vsix and centrim-docgen/centrim-docgen-0.0.1.vsix files:

- `centrim-docgen/centrim-docgen-0.0.1.vsix`:
...
The updated code includes additional options for customizing the system documentation generation, such as skipping the prompt after all commit docs have been generated. Additionally, the progress bar is now displayed to indicate how much of the documentaiton has been generated, making it easier to track progress. The output area in the script is also updated to include the completed documentation based on the output area of the output panel from Acquiring VsCode Api. Finally, additional error handling is implemented for cases where the system prompt cannot be displayed or the user chooses not to display it. These changes should make the documentation generation process more reliable and user-friendly for users.
...
**Author:** abhay  
**Date:** 2025-07-23T14:38:29+05:30  
**Message:** fix: fixing prompt issues

To generate optimized documentation based on the diff analysi, optimize prompt generated by the script, and include system requirements as part of a comprehensive system overview document, follow these steps:

1. Analyze the diff using the Ollama API and send notifications to the specified email address(es) or file paths with the analysis results.
2. Review the generated prompt and add information on system dependencies, setup/configuration requirements, key business logic, system capabilities, and functionalities as part of a comprehensive overview document for developers joining the project.
3. Follow Laravel/Supabase documentation style in creating a system overview document. Use clear heading, bullet points, and scannable text to make it practical and actionable for devs.
4. Organize information on different sections based on the system's architecture, features, APIs, database, etc., as recommended by Supabase.
5. Include setup/configuration requirements in a separate section so that developers can refer to them later if needed.
6. Follow best practices for creating clear and concise system documentation, such as using subheadings, bullet points, and code examples where appropriate.
7. Test the generated documentation by reviewing it with devs who may not be familiar with the system's architecture or requirements.
8. Revise the document if needed based on feedback and incorporate any necessary updates or corrections.
9. Send a confirmation email or notification to the specified email address(es) or file paths for successful completion of the project, including documentation links.
